...
#'   }
#'   For Single Nucleotide Variants (SNVs), \code{loc.start} and \code{loc.end} should be the same. For Copy Number Alterations (CNAs) such as gains and deletions, these fields represent the lesion’s start and end positions (lesion boundary). Structural rearrangements (e.g., translocations, inversions) should be represented by two entries (two separate rows), one for each breakpoint. An example dataset is available in the GRIN2 package (`lesion_data.rda`).
#'
...
#'   \item{An FDR-adjusted q-value using the Pounds & Cheng (2006) π̂ method}
